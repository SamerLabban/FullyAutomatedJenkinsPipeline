        ...
        stage ('Deployments'){
            parallel{
                stage ('Deploy to China'){
                    steps {                    
                        sh "scp -r -i /var/lib/jenkins/.ssh/tomcat.pem /var/lib/jenkins/workspace/FullyAutomated/webapp/target/*.war ec2-user@${params.tomcat_dev}:/var/lib/tomcat7/webapps"
                    }
                }
